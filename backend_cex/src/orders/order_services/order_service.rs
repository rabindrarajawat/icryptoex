...
// orders/order_services/order_service.rs
use crate::orders::order_dto::order_dto::{NewOrder, Order, OrderResponse};
use crate::schema::orders::dsl::*;
...
pub async fn create_order(
    state: web::Data<AppState>,  // Use AppState here
    new_order: web::Json<NewOrder>,
) -> HttpResponse {
...
pub async fn get_all_orders(state: web::Data<AppState>) -> HttpResponse {  // Use AppState here
    let mut conn = state.pool.get().expect("couldn't get db connection from pool");
